...
func TestSPUB(t *testing.T) {
	...
	nsq.ScheduledPublish(topicName, time.Now().Add(5*time.Millisecond), msgBody).WriteTo(conn)
	resp, _ := nsq.ReadResponse(conn)
	frameType, data, _ := nsq.UnpackResponse(resp)
	t.Logf("frameType: %d, data: %s", frameType, data)
	test.Equal(t, frameTypeResponse, frameType)
	...
	// duration out of range
	now := time.Now()
	schedule := now.Add(1 * time.Second).Add(opts.MaxSchTimeout)
	nsq.ScheduledPublish(topicName, schedule, make([]byte, 100)).WriteTo(conn)
	resp, _ = nsq.ReadResponse(conn)
	frameType, data, _ = nsq.UnpackResponse(resp)
	t.Logf("frameType: %d, data: %s", frameType, data)
	test.Equal(t, frameTypeError, frameType)
	test.Equal(t, fmt.Sprintf("E_INVALID SPUB schedule %d out of range 0-%d",
		schedule.UnixNano()/int64(time.Second),
		now.Add(opts.MaxSchTimeout).UnixNano()/int64(time.Second),
	), string(data))
}
