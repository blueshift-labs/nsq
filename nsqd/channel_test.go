...
// ensure that we can push a message through a topic and get it out of a channel
func TestPutMessageBadgerq(t *testing.T) {
	opts := NewOptions()
	opts.Logger = test.NewTestLogger(t)
	_, _, nsqd := mustStartNSQD(opts)
	defer os.RemoveAll(opts.DataPath)
	...
	b := <-channel1.backend.ReadChan()
	outputMsg, _ := decodeMessage(b)
	test.Equal(t, msg.ID, outputMsg.ID)
	test.Equal(t, msg.Body, outputMsg.Body)
}
...
func TestPutMessage2Chan(t *testing.T) {
	...
	b := <-channel2.backend.ReadChan()
	outputMsg2, _ := decodeMessage(b)
...
func TestChannelEmptyBadgerq(t *testing.T) {
	opts := NewOptions()
	opts.Logger = test.NewTestLogger(t)
	_, _, nsqd := mustStartNSQD(opts)
	defer os.RemoveAll(opts.DataPath)
...
func TestChannelEmptyConsumerBadgerq(t *testing.T) {
	opts := NewOptions()
	opts.Logger = test.NewTestLogger(t)
	tcpAddr, _, nsqd := mustStartNSQD(opts)
	defer os.RemoveAll(opts.DataPath)
...
func TestChannelHealthBadgerq(t *testing.T) {
	opts := NewOptions()
	opts.Logger = test.NewTestLogger(t)
	opts.MemQueueSize = 2
